...
hideAllWidgetsAndNotifyWhenOfflineElse: (callback) ->
  if window.components.weather?.find(".connection-status").length != 1
    window.components.weather.find(".connection-status").remove()
    window.components.weather.append("<span class='connection-status'></span>")
  ...
  @run @ip_command, (stderr, stdout) =>
    if not stdout? or $.trim(stdout) is "OFFLINE"
      ...
      window.components.weather.find(".connection-status").html("Currently, OFFLINE!")
      for widget in @conn_widgets
        if @isVisible(widget) then @toggleComponent(widget, "fadeOut")
    else
      window.conn_status?.close()
      window.components.weather.find(".connection-status").html("Connected as #{stdout}")
